...
public class Collection08Main {
	...
	public static void main(String[] args) {
		...
		//itr.next();  // NoSuchElementException 예외 ~!
		
		// 데이터 삭제
		// remove(element): Set에 있는 element를 찾아서 삭제
		//   element 있다면 삭제 후 true를 리턴
		//   element 없다면 false 리턴
		System.out.println("삭제결과: " + set.remove(2));  // index 2가 아니라 데이터 2 를 제거
		System.out.println("삭제결과: " + set.remove(999));
		
		System.out.println();
		System.out.println("삭제 후:");
		itr = set.iterator();  // 새로운 Iterator 객체 생성
		while(itr.hasNext()) {
			System.out.println(itr.next());
		}
